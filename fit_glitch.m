...
dirCodeDeglitch = strcat(dirDROP,"DeglitchPkg_PL_GS/");
outputdir       = strcat(dirCodeDeglitch,"/Catalogue/");
...
dirU            = [dirDROP origindir 'matlab_data/vel_sci/2sps/hg/vbbue'];  % Read only dir
dirV            = [dirDROP origindir 'matlab_data/vel_sci/2sps/hg/vbbve'];  % Read only dir
dirW            = [dirDROP origindir 'matlab_data/vel_sci/2sps/hg/vbbwe']; % Read only dir
...
dist_min_glitch = 3/dt;               % threshold to merge extrema from different axes into a single one
...
var_thres = 0.055;   % Variance threshold to cut high variance glitches
%var_thres = 0.11;   
...
plot_fit             = 1;                        % figure(400) to figure(420)
...
for iev=solstart:solend
                    ...
                    if length(reduced_glitch_vec)>=1
                        ...
                        if param_fit_glitch == 1                                    % Flag to skip the fit
                            ...
                            if length(single_extrema_list)>=1
                                ...
                                [struct_data, Glitch_cat]     =  fit_list_of_single_gliches(struct_data, index_list_2_deglitch, ...
                            ...
                            % SECOND, We are dealing with the multiple glitches
                             if length(multi_extrema_list)>=2
                                ...
                                if polyfit == 0
                                    ...
                                    [struct_data, Glitch_cat]     = fit_list_of_multi_gliches_v3(struct_data, clusters_of_glitches, ...
                                                                                                           max_glitch_per_axis, green_fn_list, ...
    ...
    if save_param == 1
        ...
        glitchfile = strcat(outputdir, "SOL",num2str(iev, "%04d"), "_Glitches_cat.mat");
        ...
        outdatafile = strcat(dirCodeDeglitch, "Output/", "SOL",num2str(iev, "%04d"), "_outdata.mat");
